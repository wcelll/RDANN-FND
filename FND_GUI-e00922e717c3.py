...
def load_model(model_path, args):
    ...
    # 添加 map_location 参数以在 CPU 上加载模型
...
def predict(model, text, mask):
    model.eval()
    with torch.no_grad():
        text = torch.tensor(text, dtype=torch.long).view(1, -1)
        mask = torch.tensor(mask, dtype=torch.float32).view(1, -1)
        empty_image_input = torch.zeros((1, 3, 224, 224), dtype=torch.float32)
...
def predict_image(model, image):
    model.eval()
    with torch.no_grad():
        empty_text_input = torch.zeros((1, args.sequence_length), dtype=torch.long)
        empty_mask_input = torch.zeros((1, args.sequence_length), dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    train = ''
    test = ''
    output = '/mnt/PyCharm_Project_1/BDANN-IJCNN2020-main/Data/weibo/output_image_text13/'
