...
def stopwordslist(filepath='D:/RDANN-FND/Data/weibo/baidu_stopwords.txt'):
    stopwords = {}
    ...
    # def clean_str_sst(string):   原代码
    """
    Tokenization/string cleaning for the SST dataset
    """


#    string = re.sub(u"[，。 :,.；|-“”——_/nbsp+&;@、《》～（）())#O！：【】]", "", string)  原代码
#    return string.strip().lower()  原代码
##我的修改部分######################
...
def read_image():
    image_list = {}
    file_list = ['D:/RDANN-FND/Data/weibo/nonrumor_images/',
                 'D:/RDANN-FND/Data/weibo/rumor_images/']
...
def write_txt(data):
    f = open("D:/RDANN-FND/Data/weibo/top_n_data.txt", 'wb')
    for line in data:
        for l in line:
            f.write(l + "\n")
        f.write("\n")
...
def write_data(flag, image, text_only):
    def read_post(flag):
        ...
        pre_path = "D:/RDANN-FND/Data/weibo/tweets/"
        file_list = [pre_path + "test_nonrumor.txt", pre_path + "test_rumor.txt",
                     pre_path + "train_nonrumor.txt", pre_path + "train_rumor.txt"]
        if flag == "train":
            id = pickle.load(open("D:/RDANN-FND/Data/weibo/train_id.pickle", 'rb'))
        elif flag == "validate":
            id = pickle.load(open("D:/RDANN-FND/Data/weibo/validate_id.pickle", 'rb'))
        elif flag == "test":
            id = pickle.load(open("D:/RDANN-FND/Data/weibo/test_id.pickle", 'rb'))
        deleted_data = pickle.load(open("D:/RDANN-FND/Data/weibo/deleted_post_fold_1.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(
            open("D:/RDANN-FND/Data/weibo/deleted_post_fold_2.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(
            open("D:/RDANN-FND/Data/weibo/deleted_post_fold_3.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(
            open("D:/RDANN-FND/Data/weibo/deleted_post_fold_4.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(
            open("D:/RDANN-FND/Data/weibo/deleted_post_fold_5.pkl", 'rb'))
    ...
    def paired(text_only=False):
        ...
        ordered_event = np.array(ordered_event, dtype=np.int64)
